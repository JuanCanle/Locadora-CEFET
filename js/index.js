...
function ShowHideCards(indicePagina, limite, paginasExistentes) {
  ...
  document.getElementById("pag" + indicePagina).style.border =
    "2px solid black";
  ...
  for (
    iteradorCardsInicio;
    iteradorCardsInicio <= iteradorCardsFim;
    iteradorCardsInicio++
  ) {
    document.getElementById("Card" + iteradorCardsInicio).style.display =
      "flex";
...
function SearchShow(indice, limite) {
  ...
  for (ArrayInicio; ArrayInicio < ArrayFim; ArrayInicio++) {
    if (ArrayInicio == ArrayCarros.length - 1) {
      //acredito que este if pode ser removido ao adicionar ArrayFim = Math.min(ArrayFim, ArrayCarros.length) anteriormente ao codigo, fonte gabriel paulo torres
      break;
    }
...
function modalclick_locacao(placa) {
  //abre o modal
